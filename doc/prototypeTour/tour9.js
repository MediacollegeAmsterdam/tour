...
function createPage(instr)
{
	...
	//str+='<img id="main" src="" />';
	str+='<div class="map" id="map_back"><canvas id="map" width=100 height=120></canvas></div>';
	str+='</div>';
	...
	// show the map back!
	var map_back=document.getElementById("map_back");
	map_back.style.backgroundImage="url(images_minecraft/map_back.png)";
	map_back.style.backgroundSize="cover";
}
function right()
{
	var old=tour.dir;
...
function drawMap()
{
	...
	// show the dir
	ctx.strokeStyle = '#0f0';
	var dirs=[{x:1,y:0},{x:0.71,y:-0.71},{x:0,y:-1},{x:-0.71,y:-0.71},{x:-1,y:0},{x:-0.71,y:0.71},{x:0,y:1},{x:0.71,y:0.71}]; // don't know if this is correct!
...
function loadMain(img_src)
{
	...
	div.style.backgroundImage = "url(images_minecraft/"+img_src+")";
	setFeedback(nodes[tour.node].img[tour.dir]);
}
