...
//    var kandraaien = true;
	...
	*/
	...
	//rond kijken met WASD code
	window.addEventListener("keydown", checkKeyPressed, false);
	var speedButtons = 0.9;
	...
	function checkKeyPressed(e) {
		...
		if (panoSphere.getSphereMode()==true) {
			if (e.keyCode == "87") {
			//w
			rotx = rotx + speedButtons;
			object3D.style.transition = "0.2s";
...
if (e.keyCode == "65"){
	...
	if (autoPanCheck == true) {
	clearTimeout(autopantimes);
	var autopantimes =	setTimeout(function () {
			panoSphere.options.autoPan=true;
	}, 10000);

	clearTimeout(rotationsmoother);
	var rotationsmoother = setTimeout(function () {
		object3D.style.transition = "0s";
	}, 200);
